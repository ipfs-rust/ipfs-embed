...
fn time_alias(db: &StorageService<DefaultParams>, old: Option<&Cid>, new: Option<&Cid>, n: u64) -> Duration {
...
fn alias(c: &mut Criterion) {
    ...
    c.bench_function("alias 1024 blocks", |b| {
        b.iter_custom(|n| {
            time_alias(&db, None, Some(prev.cid()), n)
        });
    });
    ...
    c.bench_function("alias 1024 blocks", |b| {
        b.iter_custom(|n| {
            time_alias(&db, Some(checkpoint.cid()), Some(prev.cid()), n)
        });
    });
    let checkpoint = prev.clone();
    async_std::task::block_on(async {
